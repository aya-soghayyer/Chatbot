  ...
  return (
    <>
      ...
      >
        ...
        >
          <div
            className={`hidden md:flex md:felx-row md:items-start w-screen h-full gap-10`}
          >
            <Link to="/" className="w-20 h-8 md:w-28 md:h-10">
              <img src={Logo} alt="MiLo Logo" className="w-full h-full" />
            </Link>
            <div className={`w-14 mt-3 flex justify-between`}>
          ...
          {!chatHistory && (
            <>
              <div className="h-screen md:h-full md:flex md:flex-col md:justify-start md:items-start md:gap-5">
                <h1 className="text-xl mt-3 md:text-lg md:items-start md:font-normal text-white/55 mb-3 md:mb-0">
                  History
                </h1>

                <div className="h-3/4 md:h-screen md:w-full md:grid md:items-start">
                  {loading ? (
                    <p className="text-gray-400 ">Loading chats...</p>
                  ) : error ? (
                    <p className="text-red-500">{error}</p>
                  ) : chats.length > 0 ? (
                    <ul className="space-y-6 md:space-y-1 overflow-y-auto max-h-[470px] md:max-h-[460px] custom-scrollbar px-1">
                      {chats.map((chat) => (
                        <li
                ...
                <div className="md:hidden">
                  ...
                  <div className="flex justify-between items-center mt-4 px-3">
                    ...
                    <button onClick={() => setSettings(!showSettings)}>
                      <img
                        src={userMobileSetting}
                        alt="settings for the user"
                      />
                    </button>
                    <Settings
                      className="z-30"
                      ...
                      setSettingsDetails={setSettingsDetails}
